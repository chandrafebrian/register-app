pipeline {
    ...
    stages{
       ...
       stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user chandrajenkinsadmin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'http://34.142.229.151:8080/job/gitops-register-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
       }
    }
