pipeline {
    ...
    stages{
       ...
       stage("Trigger CD Automate Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user chandrajenkinsadmin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' '35.213.144.36/:8080/job/gitops-register-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
       }
